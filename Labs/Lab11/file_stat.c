...
int main(int argc, char *argv[]) {
  ...
  if (fstat(fd, &file_info) == 0) {
    ...
    //struct passwd stuff = malloc(sizeof(struct passwd));
     ...
     struct group *result2 = malloc(sizeof(struct group));
     result = getpwuid(file_info.st_uid);
     result2 = getgrgid(file_info.st_gid);
    printf(" owner user name = %s\n",result->pw_name); // TO-DO: man getpwuid 
    printf(" owner group name = %s\n", result2->gr_name); // TO-DO: man getgrgid 
